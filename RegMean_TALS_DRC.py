...
def construct_base_model(model_name):
    ...
    if "t5" in model_name_l:
        ...
        raise ValueError(f"[RegMean_TALS_DRC] Unsupported model_name: {model_name}")


def load_required_regmean_coarse_model(model_name, model_dir, model_label="Linear"):
    """
    ...
    不允许静默重新做 RegMean merge。
    """
    if model_dir is None or str(model_dir).strip() == "":
        raise ValueError(f"[RegMean_TALS_DRC] Empty {model_label} coarse model dir.")

    model_dir = str(model_dir)

    if not os.path.isdir(model_dir):
        raise FileNotFoundError(
            f"[RegMean_TALS_DRC] Required {model_label} coarse model dir not found: {model_dir}\n"
            f"Please run Linear.py first with save=1."
        )

    if not os.path.exists(os.path.join(model_dir, "config.json")):
        raise FileNotFoundError(
            f"[RegMean_TALS_DRC] {model_label} coarse model dir exists but config.json is missing: {model_dir}\n"
            f"This does not look like a valid HuggingFace saved dense model directory."
        )

    print(f"[RegMean_TALS_DRC] Loading required {model_label} coarse model from: {model_dir}")
...
def collect_lora_input_features(
    ...
    for key in lora_keys:
        if key not in modules:
            print(f"[RegMean_TALS_DRC][Warn] Cannot find module for hook: {key}")
            continue
...
def collect_encoder_block_output_features(
    ...
    for key in block_keys:
        if key not in modules:
            print(f"[RegMean_TALS_DRC][Warn] Cannot find encoder block for hook: {key}")
            continue
...
def select_drc_targets(
    ...
    if inject_position == "lora_input":
        ...
        print(f"[RegMean_TALS_DRC] drc_inject_position = lora_input")
        print(f"[RegMean_TALS_DRC] Found {len(all_lora_keys)} LoRA target modules.")
        print(f"[RegMean_TALS_DRC] Selected {len(selected_target_keys)} modules for DRC injection.")
        ...
        print(f"[RegMean_TALS_DRC] drc_inject_position = encoder_block_output")
        print(f"[RegMean_TALS_DRC] Selected {len(selected_target_keys)} encoder blocks for DRC injection.")
    ...
    for key in selected_target_keys[:10]:
        print(f"[RegMean_TALS_DRC]   target: {key}")
    if len(selected_target_keys) > 10:
        print(f"[RegMean_TALS_DRC]   ... {len(selected_target_keys) - 10} more")
...
def load_adapter_meta(lora_dir, default_rank=8, default_alpha=32):
    ...
    if os.path.exists(cfg_path):
        try:
            ...
            print(f"[RegMean_TALS_DRC][Warn] Failed to read adapter_config.json from {lora_dir}: {e}")
...
def load_lora_delta_matrices(lora_dir, target_keys, default_rank=8, default_alpha=32):
    ...
    if safe_load_file is None:
        raise ImportError(
            "safetensors is required for RegMean_TALS_DRC, but safetensors.torch.load_file is unavailable."
        )
    ...
    for target_key in target_keys:
        ...
        if A.dim() != 2 or B.dim() != 2:
            print(
                f"[RegMean_TALS_DRC][Warn] Invalid LoRA tensor dim at {target_key}: "
        ...
        if B.size(1) != A.size(0):
            print(
                f"[RegMean_TALS_DRC][Warn] LoRA shape mismatch at {target_key}: "
                f"A={tuple(A.shape)}, B={tuple(B.shape)}"
            )
            continue
    ...
    if missing:
        print(f"[RegMean_TALS_DRC][Warn] Missing LoRA A/B for {len(missing)} targets in {lora_dir}.")
...
def build_all_task_lora_deltas(task_targets, lora_path_dict, target_keys, rank, lora_alpha_list=None):
    ...
    for idx, task_name in enumerate(task_targets):
        ...
        print(f"[RegMean_TALS_DRC] Load LoRA ΔW matrices for task={task_name}...")
...
def compute_merged_delta_for_key(all_lora_deltas, task_targets, linear_weights, key):
    """
    计算 RegMean merged LoRA ΔW_c = sum_j λ_j ΔW_j。
    """
    merged_delta = None
...
def build_dense_coarse_delta_dict(model_name, coarse_model, target_keys):
    """
    For RegMean coarse merged model, reconstruct dense coarse update:

        ΔW_c = W_RegMean - W_base

    This differs from Linear_TALS_DRC, where ΔW_c can be obtained by
    weighted averaging of LoRA deltas. RegMean is data-dependent, so we
    use the actual saved dense RegMean model.
    """
    print("[RegMean_TALS_DRC] Build dense RegMean coarse ΔW_c = W_RegMean - W_base ...")
    ...
    print(
        f"[RegMean_TALS_DRC] Built dense RegMean coarse deltas for "
        f"{len(coarse_delta_dict)}/{len(target_keys)} targets."
    )
    if missing_report:
        print(f"[RegMean_TALS_DRC][Warn] Missing/mismatched dense coarse weights: {len(missing_report)}")
...
def select_drc_targets_any(
    ...
    if inject_position != "lora_input":
        raise ValueError("[RegMean_TALS_DRC][VLM] BLIP currently supports only drc_inject_position='lora_input'.")

    first_task = task_targets[0]
    adapter_file = os.path.join(lora_path_dict[first_task], "adapter_model.safetensors")
    if not os.path.exists(adapter_file):
        raise FileNotFoundError(f"[RegMean_TALS_DRC][VLM] adapter_model.safetensors not found: {adapter_file}")

    state_dict = safe_load_file(adapter_file) if safe_load_file is not None else None
    if state_dict is None:
        raise ImportError("[RegMean_TALS_DRC][VLM] safetensors.torch.load_file is required.")
    ...
    for full_key in sorted(state_dict.keys()):
        ...
        if base_name not in model_modules:
            print(f"[RegMean_TALS_DRC][VLM][Warn] LoRA target not found in dense BLIP modules: {base_name}")
            continue

        target_keys.append(base_name)

    target_keys = sorted(set(target_keys))
    print(f"[RegMean_TALS_DRC][VLM] Selected {len(target_keys)} BLIP DRC targets.")
    for key in target_keys[:20]:
        print(f"[RegMean_TALS_DRC][VLM]   target: {key}")
    if len(target_keys) > 20:
        print(f"[RegMean_TALS_DRC][VLM]   ... {len(target_keys) - 20} more")

    if len(target_keys) == 0:
        raise RuntimeError(
            "[RegMean_TALS_DRC][VLM] No BLIP DRC targets selected. "
...
def collect_features_by_position_any(
    ...
    if inject_position != "lora_input":
        raise ValueError("[RegMean_TALS_DRC][VLM] Only drc_inject_position='lora_input' is supported for BLIP.")
    ...
    for key in target_keys:
        if key not in modules:
            print(f"[RegMean_TALS_DRC][VLM][Warn] Cannot find module for hook: {key}")
            continue
    ...
    for key, value in features.items():
        print(f"[RegMean_TALS_DRC][VLM][FeatureShape] {key}: {tuple(value.shape)}")
        shown += 1
        if shown >= 3:
            break
...
def build_task_specific_drc_directions(
    ...
    对每个任务单独构造 TALS-DRC 方向:
        ...
        r_act = h_single - h_regmean

    权重子空间:
        missing: R_W = ΔW_task - ΔW_regmean
        single : R_W = ΔW_task
    ...
    if inject_position != "lora_input":
        raise ValueError(
            "RegMean_TALS_DRC currently supports only drc_inject_position='lora_input', "
    ...
    coarse_delta_dict = build_dense_coarse_delta_dict(
        model_name=model_name,
        coarse_model=linear_model,
        target_keys=selected_target_keys,
    )
    ...
    for task_name in task_targets:
        print(f"\n[RegMean_TALS_DRC] Building task-specific DRC direction for task = {task_name}")
        ...
        print(f"[RegMean_TALS_DRC] Collect base features on {task_name} samples...")
        ...
        print(f"[RegMean_TALS_DRC] Collect single-LoRA features for {task_name}...")
        ...
        # 3. RegMean merged model features
        print(f"[RegMean_TALS_DRC] Collect RegMean merged features on {task_name} samples...")
        if is_blip_model(model_name):
            regmean_features = collect_features_by_position_any(
                model=linear_model,
                model_name=model_name,
                inject_position=inject_position,
                target_keys=selected_target_keys,
        ...
        for key in selected_target_keys:
            ...
            if key not in single_features:
                ...
                continue
            if key not in regmean_features:
                missing_report["missing_regmean"].append(key)
                continue

            # activation residual: h_single - h_regmean
            single_shift = single_features[key] - base_features[key]
            regmean_shift = regmean_features[key] - base_features[key]
            delta = single_shift - regmean_shift
            ...
            if key not in coarse_delta_dict:
                missing_report["missing_lora_delta"].append(("regmean_dense_delta", key))
                continue
            ...
            random_seed = seed + stable_int_hash(f"regmean|{task_name}|{key}|{tals_rank}")
            ...
            if tals_delta is None:
                ...
                if tals_fallback_to_base:
                    print(
                        f"[RegMean_TALS_DRC][Warn] TALS filter failed at {key}; "
        ...
        print(f"[RegMean_TALS_DRC][Debug] Missing/skip report for task={task_name}:")
        ...
        print(
            f"[RegMean_TALS_DRC] Task {task_name}: built directions for "
        ...
        if tals_use_layer_weight and len(task_direction) > 0:
            ...
            print(f"[RegMean_TALS_DRC][LayerWeight] task={task_name}, summary={lw_summary}")
...
def register_drc_pre_hooks(
    ...
    def make_pre_hook(layer_name, direction):
        def pre_hook(module, inputs):
            ...
            if x.size(-1) != direction.numel():
                print(
                    f"[RegMean_TALS_DRC][Warn] Shape mismatch at {layer_name}: "
    ...
    for layer_name, direction in directions.items():
        if layer_name not in modules:
            print(f"[RegMean_TALS_DRC][Warn] Cannot find module for DRC hook: {layer_name}")
            continue
    ...
    print(f"[RegMean_TALS_DRC] Registered {len(handles)} LoRA input DRC pre-hooks.")
    return handles
...
def register_encoder_block_output_hooks(
    ...
    for layer_name, direction in directions.items():
        if layer_name not in modules:
            print(f"[RegMean_TALS_DRC][Warn] Cannot find encoder block for DRC hook: {layer_name}")
            continue
    ...
    print(f"[RegMean_TALS_DRC] Registered {len(handles)} encoder block output DRC hooks.")
    return handles
...
def main():
    ...
    # Kept only for compatibility with shared helper signatures and registry schema.
    linear_weights = task_cfg.get("linear_weights", [1.0 / len(task_targets)] * len(task_targets))

    # LoRA 来源选择。RegMean_TALS_DRC 默认使用与 RegMean.py 相同的 LoRA 来源。
    # GLUE_t5: best_LoRA/T5-xxx-LoRA
    # TASKS_blip_base: loras/SENTICAP-lora-blip/{positive,negative}
    ...
    if lora_source in ["default", "best", "normal", "gaussian"]:
        ...
        coarse_prefix_default = "merged_model/RegMean_"
        default_method_name = "RegMean_TALS_DRC_LER_BLIP" if task_type == "TASKS_blip_base" else "RegMean_TALS_DRC_LER"
        coarse_label = "RegMean"
    elif lora_source == "osrm":
        # 预留：如果后续有 OSRM_LoRA + RegMean 的 coarse model，可复用此分支。
        lora_root = task_cfg.get("lora_root", "OSRM_LoRA")
        coarse_prefix_default = "merged_model/OSRM_RegMean_"
        default_method_name = "OSRM_RegMean_TALS_DRC_LER"
        coarse_label = "OSRM_RegMean"
    else:
        # 也允许直接把 lora_source 写成一个目录名。
        lora_root = task_cfg.get("lora_root", lora_source)
        coarse_prefix_default = task_cfg.get("regmean_tals_coarse_model_prefix", "merged_model/RegMean_")
        default_method_name = "RegMean_TALS_DRC_LER_BLIP" if task_type == "TASKS_blip_base" else "RegMean_TALS_DRC_LER"
        coarse_label = str(lora_source)

    print(f"[RegMean_TALS_DRC] lora_source = {lora_source}")
    print(f"[RegMean_TALS_DRC] lora_root = {lora_root}")
    ...
    for task in task_targets:
        ...
        if not os.path.exists(adapter_path):
            raise FileNotFoundError(
                f"[RegMean_TALS_DRC] Cannot find adapter_model.safetensors for task={task}: {adapter_path}\n"
    ...
    regmean_tals_load_coarse_from_dir = bool(
        task_cfg.get("regmean_tals_load_coarse_from_dir", True)
    )

    existing_prefix = str(
        task_cfg.get("regmean_tals_existing_coarse_prefix", "")
    ).strip()
    ...
    regmean_tals_coarse_model_dir = task_cfg.get(
        "regmean_tals_coarse_model_dir",
        task_cfg.get("regmean_merged_model_dir", default_coarse_model_dir),
    )

    if not regmean_tals_load_coarse_from_dir:
        raise ValueError(
            "[RegMean_TALS_DRC] regmean_tals_load_coarse_from_dir must be True. "
            "This script should load an existing RegMean coarse model, "
            "not rebuild Linear inside RegMean_TALS_DRC.py."
        )

    merged_model_dir = regmean_tals_coarse_model_dir

    print(f"[RegMean_TALS_DRC] regmean_tals_load_coarse_from_dir = {regmean_tals_load_coarse_from_dir}")
    print(f"[RegMean_TALS_DRC] Expected coarse model dir = {merged_model_dir}")

    model = load_required_regmean_coarse_model(
        model_name=model_name,
        model_dir=merged_model_dir,
        model_label=coarse_label,
    ).to("cuda")

    print("[RegMean_TALS_DRC] Loaded saved coarse model. No RegMean merge is run in this script.")
    ...
    cache_dir = task_cfg.get("regmean_tals_cache_dir", task_cfg.get("drc_cache_dir", "direction_cache"))
    ...
    print(f"[RegMean_TALS_DRC] drc_inject_position = {drc_inject_position}")
    print(f"[RegMean_TALS_DRC] drc_alpha = {drc_alpha}")
    print(f"[RegMean_TALS_DRC] drc_alpha_search = {drc_alpha_search}")
    print(f"[RegMean_TALS_DRC] drc_alpha_candidates = {drc_alpha_candidates}")
    print(f"[RegMean_TALS_DRC] drc_samples_per_task = {drc_samples_per_task}")
    print(f"[RegMean_TALS_DRC] drc_target_part = {drc_target_part}")
    print(f"[RegMean_TALS_DRC] drc_target_modules = {drc_target_modules}")
    print(f"[RegMean_TALS_DRC] drc_target_layers = {drc_target_layers}")
    print(f"[RegMean_TALS_DRC] drc_normalize_direction = {drc_normalize_direction}")
    print(f"[RegMean_TALS_DRC] tals_rank = {tals_rank}")
    print(f"[RegMean_TALS_DRC] tals_gamma = {tals_gamma}")
    print(f"[RegMean_TALS_DRC] tals_weight_norm = {tals_weight_norm}")
    print(f"[RegMean_TALS_DRC] tals_svd_center = {tals_svd_center}")
    print(f"[RegMean_TALS_DRC] tals_subspace_source = {tals_subspace_source}")
    print(f"[RegMean_TALS_DRC] tals_fallback_to_base = {tals_fallback_to_base}")
    print(f"[RegMean_TALS_DRC] tals_use_layer_weight = {tals_use_layer_weight}")
    print(f"[RegMean_TALS_DRC] tals_layer_weight_score = {tals_layer_weight_score}")
    print(f"[RegMean_TALS_DRC] tals_layer_weight_norm = {tals_layer_weight_norm}")
    print(f"[RegMean_TALS_DRC] tals_layer_weight_clip_min = {tals_layer_weight_clip_min}")
    print(f"[RegMean_TALS_DRC] tals_layer_weight_clip_max = {tals_layer_weight_clip_max}")
    print(f"[RegMean_TALS_DRC] cache_path = {cache_path}")

    start_time = time.time()
    torch.cuda.reset_peak_memory_stats()

    if (not drc_rebuild_cache) and os.path.exists(cache_path):
        print(f"[RegMean_TALS_DRC] Load DRC direction cache from: {cache_path}")
        ...
        print(f"[RegMean_TALS_DRC] Saved DRC direction cache to: {cache_path}")
    ...
    print(f"[RegMean_TALS_DRC] results_dir = {results_dir}")
    ...
    registry_header = [
        ...
        "task_targets", "coarse_cfg", "rank", "lora_alpha",
    ...
    try:
        for task_name in task_targets:
            print(f"\n[Eval] Evaluating RegMean + task-specific TALS-DRC on {task_name}...")
            ...
            for alpha in candidate_alphas:
                print(f"[RegMean_TALS_DRC][AlphaSearch] task={task_name}, alpha={alpha}")
            ...
            print(
                f"[RegMean_TALS_DRC][AlphaSearch][Best] task={task_name}, "
        ...
        print(f"\n[Done] RegMean_TALS_DRC finished for pair: {pair_name}")
