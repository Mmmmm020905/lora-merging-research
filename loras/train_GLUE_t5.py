...
_METRIC_TOKENIZER = AutoTokenizer.from_pretrained(LOCAL_MODEL_PATH, local_files_only=True)

def compute_metrics(eval_pred):
    preds, labels = eval_pred
    labels = np.where(labels != -100, labels, _METRIC_TOKENIZER.pad_token_id)
    labels = labels[:, 0]
    preds = preds[:, 1]
