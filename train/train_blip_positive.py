...
dataset = load_dataset("data/SENTICAP/positive")
dataset = DatasetDict({
    'train': dataset['train'],
    'test': dataset['test'],
})
...
class ImageCaptioningDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        encoding = self.processor(
            images=item["image"], 
            text=f"a photo that expresses positive sentiments{positive_prompt} of "+item["text"], 
            padding="max_length", 
            return_tensors="pt", 
            max_length=168,
        )
...
class ImageCaptioningDataset_val(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text_list = [f"a photo that expresses positive sentiments{positive_prompt} of "+text for text in json.loads(item["text"])]
        if len(text_list) < 3:
        ...
        if idx % int(len(train_dataloader)/4) == int(len(train_dataloader)/4)-1:
            ...
            for _, val_batch in tqdm(enumerate(val_dataloader), total=len(val_dataloader), desc="Processing", unit="val_batch"):
                ...
                val_inputs = processor(
                    text=[f"a photo that expresses positive sentiments{positive_prompt} of "]*len(pixel_values), 
                    return_tensors="pt", 
                ).to(device)
                out = model.generate(
                    **val_inputs,
            ...
            for i in range(10):
                print(preds_list[i])
            acc_scrore = [get_emotion(pred[0]) for _, pred in preds_list.items()].count(1)/len(preds_list)
            print(f"bleu: {bleu}", flush=True)
            print(f"acc: {acc_scrore}", flush=True)
            print(f"rouge: {rouge}", flush=True)
            print(f"cider: {cider}", flush=True)
            print(" ")
            saving_path = f"loras/SENTICAP-lora-blip/positive/checkpoint-{epoch*(len(train_dataloader)-1)+idx}"
